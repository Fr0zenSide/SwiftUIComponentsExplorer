...
struct SearchWithTokenView: View {
...
//        NavigationStack {
            ...
            .overlay {
                if isSearching { // put here whatever condition you need to show the suggested tokens
                    List(allTokens) { tkn in
                        Button(tkn.name) {
                            currentTokens.append(tkn)
                        }
                    }
//                    List(allTokens, id: \.id) { suggestion in
//                        Button(suggestion.name) {
//                            currentTokens.append(suggestion)
//                        }
//                    }
                }
            }
            .navigationTitle("Movies+")
            .searchable(text: $searchText, tokens: $currentTokens, suggestedTokens: .constant(suggestedTokens), isPresented: $isSearching, prompt: Text("Type to filter, or use # for tags")) { token in
                Text(token.name)
                    .offset(y: 100)
            }
        }
